...
This document covers **feature skills** — skills that add capabilities via git branch merges. This is the most complex skill type and the primary way OpalClaw is extended.

OpalClaw has four types of skills overall. See [CONTRIBUTING.md](../CONTRIBUTING.md) for the full taxonomy:
...
This replaces the previous `skills-engine/` system (three-way file merging, `.opalclaw/` state, manifest files, replay, backup/restore) with plain git operations and Claude for conflict resolution.

## How It Works

### Repository structure

The upstream repo (`qwibitai/opalclaw`) maintains:

- `main` — core OpalClaw (no skill code)
...
- `/setup`, `/debug`, `/update-opalclaw`, `/customize`, `/update-skills`
...
- Live in the marketplace repo (`qwibitai/opalclaw-skills`)
...
claude plugin install opalclaw-skills@opalclaw-skills --scope project
```
...
OpalClaw's `.claude/settings.json` registers the official marketplace:
...
```
qwibitai/opalclaw-skills/
  .claude-plugin/
    marketplace.json              # Plugin catalog
  plugins/
    opalclaw-skills/              # Single plugin bundling all official skills
      .claude-plugin/
...
Multiple skills are bundled in one plugin — installing `opalclaw-skills` makes all feature skills available at once. Individual skills don't need separate installation.
...
- Built into `/update-opalclaw` — after merging main, optionally check for skill updates
...
1. Fork `qwibitai/opalclaw` on GitHub (click the Fork button)
2. Clone your fork:
   ```bash
   git clone https://github.com/<you>/opalclaw.git
   cd opalclaw
   ```
3. Run Claude Code:
   ```bash
...
Users who previously ran `git clone https://github.com/qwibitai/opalclaw.git` and have local customizations:

1. Fork `qwibitai/opalclaw` on GitHub
2. Reroute remotes:
   ```bash
   git remote rename origin upstream
   git remote add origin https://github.com/<you>/opalclaw.git
...
3. From this point, `origin` = their fork, `upstream` = qwibitai/opalclaw
...
In both cases:
- Delete the `.opalclaw/` directory (no longer needed)
...
This is the same as the existing `/update-opalclaw` skill's merge path.

### Updating skills

Run `/update-skills` or let `/update-opalclaw` check after a core update. For each previously-merged skill branch that has new commits, Claude offers to merge the updates.
...
# Create PR from my-fix to qwibitai/opalclaw:main
```
...
4. Add the skill's SKILL.md to the marketplace repo (`qwibitai/opalclaw-skills`)
...
1. Maintains a fork of OpalClaw (e.g., `alice/opalclaw`)
2. Creates `skill/*` branches on their fork with their custom skills
3. Creates a marketplace repo (e.g., `alice/opalclaw-skills`) with a `.claude-plugin/marketplace.json` and plugin structure

### Adding a community marketplace

If the community contributor is trusted, they can open a PR to add their marketplace to OpalClaw's `.claude/settings.json`:
...
```json
{
  "extraKnownMarketplaces": {
    ...
    "alice-opalclaw-skills": {
      "source": {
        "source": "github",
        "repo": "alice/opalclaw-skills"
      }
    }
  }
}
...
Once merged, all OpalClaw users automatically discover the community marketplace alongside the official one.
...
```bash
claude plugin install alice-skills@alice-opalclaw-skills --scope project
```
...
- **Community skills use the same merge pattern.** The SKILL.md just points to a different remote:
  ```bash
  git remote add alice https://github.com/alice/opalclaw.git
  git fetch alice skill/my-cool-feature
  git merge alice/skill/my-cool-feature
  ```
- **Users can also add marketplaces manually.** Even without being listed in settings.json, users can run `/plugin marketplace add alice/opalclaw-skills` to discover skills from any source.
...
A flavor is a curated fork of OpalClaw — a combination of skills, custom changes, and configuration tailored for a specific use case (e.g., "OpalClaw for Sales," "OpalClaw Minimal," "OpalClaw for Developers").
...
AskUserQuestion: "Start with a flavor or default OpalClaw?"
- Default OpalClaw
- OpalClaw for Sales — Gmail + Slack + CRM (maintained by alice)
- OpalClaw Minimal — Telegram-only, lightweight (maintained by bob)

If a flavor is chosen:

```bash
git remote add <flavor-name> https://github.com/alice/opalclaw.git
...
- `upstream` — `qwibitai/opalclaw` (core updates)
...
- **GitHub topics** — flavor forks can tag themselves with `opalclaw-flavor` for searchability
...
- `.opalclaw/` state directory

Operational skills (`setup`, `debug`, `update-opalclaw`, `customize`, `update-skills`) remain on main in `.claude/skills/`.
...
Before:
```bash
git clone https://github.com/qwibitai/OpalClaw.git
cd OpalClaw
claude
```
...
```
1. Fork qwibitai/opalclaw on GitHub
2. git clone https://github.com/<you>/opalclaw.git
3. cd opalclaw
4. claude
5. /setup
```
...
- Check if `upstream` remote exists; if not, add it: `git remote add upstream https://github.com/qwibitai/opalclaw.git`
- Check if `origin` points to the user's fork (not qwibitai). If it points to qwibitai, guide them through the fork migration.
- **Install marketplace plugin:** `claude plugin install opalclaw-skills@opalclaw-skills --scope project` — makes all feature skills available (hot-loaded, no restart)
...
The `.claude/skills/` directory on `main` retains only operational skills (setup, debug, update-opalclaw, customize, update-skills). Feature skills (add-discord, add-telegram, etc.) live in the marketplace repo, installed via `claude plugin install` during `/setup` or `/customize`.
...
- **Marketplace repo** (`qwibitai/opalclaw-skills`) — single Claude Code plugin bundling SKILL.md files for all feature skills
...
>
> We've simplified how skills work in OpalClaw. Instead of a custom skills engine, skills are now git branches that you merge in.
>
...
> - No more `.opalclaw/` state directory or skills engine
>
...
>
> **If you previously applied skills via the old system**, your code changes are already in your working tree — nothing to redo. You can delete the `.opalclaw/` directory. Future skills and updates use the branch-based approach.
>
...
>
> **Want to run your own skill marketplace?** Maintain skill branches on your fork and create a marketplace repo. Open a PR to add it to OpalClaw's auto-discovered marketplaces — or users can add it manually via `/plugin marketplace add`.
