...
│  │    • mcp__opalclaw__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│   ├── opalclaw.log               # Host stdout
│   └── opalclaw.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.opalclaw.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'opalclaw-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your opalclaw installation
...
OpalClaw uses a hierarchical memory system based on CLAUDE.md files.
...
OpalClaw has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__opalclaw__schedule_task]
        {
...
### OpalClaw MCP (built-in)

The `opalclaw` MCP server is created dynamically per agent call with the current group's context.
...
OpalClaw runs as a single macOS launchd service.

### Startup Sequence

When OpalClaw starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned OpalClaw containers from previous runs
...
### Service: com.opalclaw

**launchd/com.opalclaw.plist:**
```xml
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/opalclaw.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/opalclaw.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.opalclaw.plist ~/Library/LaunchAgents/
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep opalclaw` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; OpalClaw auto-starts container runtime but may fail |
...
- `logs/opalclaw.log` - stdout
- `logs/opalclaw.error.log` - stderr
