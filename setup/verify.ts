...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.opalclaw')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.opalclaw'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active opalclaw`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
  ...
  if (
    fs.existsSync(
      path.join(homeDir, '.config', 'opalclaw', 'mount-allowlist.json'),
    )
  ) {
