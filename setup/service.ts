...
<dict>
    ...
    <string>${projectRoot}/logs/opalclaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/opalclaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.opalclaw');
  } catch {
...
/**
 * Kill any orphaned opalclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned opalclaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/opalclaw.service';
    ...
    unitPath = path.join(unitDir, 'opalclaw.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=OpalClaw Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/opalclaw.log
StandardError=append:${projectRoot}/logs/opalclaw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned opalclaw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable opalclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start opalclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active opalclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-opalclaw.sh');
  const pidFile = path.join(projectRoot, 'opalclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-opalclaw.sh — Start OpalClaw without systemd',
    ...
    '    echo "Stopping existing OpalClaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting OpalClaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/opalclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/opalclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "OpalClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/opalclaw.log"`,
  ];
