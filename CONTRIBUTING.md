...
1. **Check for existing work.** Search open PRs and issues before starting:
   ```bash
   gh pr list --repo qwibitai/opalclaw --search "<your feature>"
   gh issue list --repo qwibitai/opalclaw --search "<your feature>"
   ```
...
OpalClaw uses [Claude Code skills](https://code.claude.com/docs/en/skills) — markdown files with optional supporting files that teach Claude how to do something. There are four types of skills in OpalClaw, each serving a different purpose.
...
Add capabilities to OpalClaw by merging a git branch. The SKILL.md contains setup instructions; the actual code lives on a `skill/*` branch.
...
1. Fork `qwibitai/opalclaw` and branch from `main`
...
**Examples:** `/setup`, `/debug`, `/customize`, `/update-opalclaw`, `/update-skills`
