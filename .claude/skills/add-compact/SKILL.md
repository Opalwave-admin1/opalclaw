...
> **Note:** `upstream` is the remote pointing to `qwibitai/opalclaw`. If using a different remote name, substitute accordingly.
...
1. Start OpalClaw in dev mode: `npm run dev`
...
```bash
git clone <your-fork> /tmp/opalclaw-test
cd /tmp/opalclaw-test
...
- **No config file.** OpalClaw's philosophy is customization through code changes, not configuration sprawl.
