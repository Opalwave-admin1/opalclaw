---
name: update-opalclaw
description: Efficiently bring upstream OpalClaw updates into a customized install, with preview, selective cherry-pick, and low token usage.
---

# About

Your OpalClaw fork drifts from upstream as you customize it. This skill pulls upstream changes into your install without losing your modifications.

Run `/update-opalclaw` in Claude Code.

## How it works

**Preflight**: checks for clean working tree (`git status --porcelain`). If `upstream` remote is missing, asks you for the URL (defaults to `https://github.com/qwibitai/opalclaw.git`) and adds it. Detects the upstream branch name (`main` or `master`).
...
# Goal
Help a user with a customized OpalClaw install safely incorporate upstream changes without a fresh reinstall and without blowing tokens.
...
- Ask the user for the upstream repo URL (default: `https://github.com/qwibitai/opalclaw.git`).
...
- Restart the service to apply changes:
  - If using launchd: `launchctl unload ~/Library/LaunchAgents/com.opalclaw.plist && launchctl load ~/Library/LaunchAgents/com.opalclaw.plist`
  - If running manually: restart `npm run dev`


## Diagnostics

1. Use the Read tool to read `.claude/skills/update-opalclaw/diagnostics.md`.
