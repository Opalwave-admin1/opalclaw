...
This skill switches OpalClaw's container runtime from Docker to Apple Container (macOS-only). It uses the skills engine for deterministic code changes, then walks through verification.
...
```bash
echo '{}' | container run -i --entrypoint /bin/echo opalclaw-agent:latest "Container OK"
```
