...
description: Add a macOS menu bar status indicator for OpalClaw. Shows a bolt icon with a green/red dot indicating whether OpalClaw is running, with Start, Stop, and Restart controls. macOS only.
---

# Add macOS Menu Bar Status Indicator

Adds a persistent menu bar icon that shows OpalClaw's running status and lets the user
...
```bash
launchctl list | grep com.opalclaw.statusbar
```
...
Create `~/Library/LaunchAgents/com.opalclaw.statusbar.plist`, substituting the actual values
...
<dict>
    <key>Label</key>
    <string>com.opalclaw.statusbar</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.opalclaw.statusbar.plist
```
...
> The bolt icon should now appear in your macOS menu bar. Click it to see OpalClaw's status and control the service.
>
> - **Green dot** — OpalClaw is running
> - **Red dot** — OpalClaw is stopped
>
...
```bash
launchctl unload ~/Library/LaunchAgents/com.opalclaw.statusbar.plist
rm ~/Library/LaunchAgents/com.opalclaw.statusbar.plist
rm dist/statusbar
```
