...
class StatusBarController: NSObject {
    ...
    private let plistPath = "\(NSHomeDirectory())/Library/LaunchAgents/com.opalclaw.plist"

    /// Derive the OpalClaw project root from the binary location.
    ...
    private func setupStatusItem() {
        ...
        if let button = statusItem.button {
            if let image = NSImage(systemSymbolName: "bolt.fill", accessibilityDescription: "OpalClaw") {
                image.isTemplate = true
                button.image = image
            } else {
                button.title = "⚡"
            }
            button.toolTip = "OpalClaw"
        }
    }
    ...
    private func checkRunning() -> Bool {
        ...
        task.arguments = ["list", "com.opalclaw"]
        let pipe = Pipe()
    ...
    private func updateMenu() {
        ...
        let label = isRunning ? "OpalClaw is running" : "OpalClaw is stopped"
    ...
    @objc private func restartService() {
        let uid = getuid()
        run("/bin/launchctl", ["kickstart", "-k", "gui/\(uid)/com.opalclaw"])
        refresh(after: 3)
    }

    @objc private func viewLogs() {
        let logPath = "\(StatusBarController.projectRoot)/logs/opalclaw.log"
