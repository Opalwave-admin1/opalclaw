---
name: add-emacs
description: Add Emacs as a channel. Opens an interactive chat buffer and org-mode integration so you can talk to OpalClaw from within Emacs (Doom, Spacemacs, or vanilla). Uses a local HTTP bridge — no bot token or external service needed.
---

# Add Emacs Channel

This skill adds Emacs support to OpalClaw, then walks through interactive setup.
...
- **Code review** — select a region and send it with `opalclaw-org-send`; the response appears as a child heading inline in your org file
...
- **Schedule tasks** — ask Andy to set a reminder or create a scheduled OpalClaw task (e.g. "remind me tomorrow to review the PR")
...
If an `upstream` remote pointing to `https://github.com/qwibitai/opalclaw.git` is missing,
add it:

```bash
git remote add upstream https://github.com/qwibitai/opalclaw.git
```
...
- `emacs/opalclaw.el` — Emacs Lisp package (`opalclaw-chat`, `opalclaw-org-send`)
...
The `opalclaw.el` package requires only Emacs 27.1+ built-in libraries (`url`, `json`, `org`) — no package manager setup needed.
...
```elisp
;; OpalClaw — personal AI assistant channel
(load (expand-file-name "~/src/opalclaw/emacs/opalclaw.el"))

(map! :leader
      :prefix ("N" . "OpalClaw")
      :desc "Chat buffer"  "c" #'opalclaw-chat
      :desc "Send org"     "o" #'opalclaw-org-send)
```
...
```elisp
;; OpalClaw — personal AI assistant channel
(load-file "~/src/opalclaw/emacs/opalclaw.el")

(spacemacs/set-leader-keys "aNc" #'opalclaw-chat)
(spacemacs/set-leader-keys "aNo" #'opalclaw-org-send)
```
...
(global-set-key (kbd "C-c n c") #'opalclaw-chat)
(global-set-key (kbd "C-c n o") #'opalclaw-org-send)
```
...
```elisp
(setq opalclaw-auth-token "<your-token>")
```

If `EMACS_CHANNEL_PORT` was changed from the default, also add:

```elisp
(setq opalclaw-port <your-port>)
```
...
```bash
npm run build
launchctl kickstart -k gui/$(id -u)/com.opalclaw  # macOS
# Linux: systemctl --user restart opalclaw
```
...
Either a stale OpalClaw process is running, or 8766 is taken by another app.
...
Or change the port in `.env` (`EMACS_CHANNEL_PORT=8767`) and update `opalclaw-port` in Emacs config.

### No response from agent

Check:
1. OpalClaw is running: `launchctl list | grep opalclaw` (macOS) or `systemctl --user status opalclaw` (Linux)
2. Emacs group is registered: `sqlite3 store/messages.db "SELECT * FROM registered_groups WHERE jid = 'emacs:default'"`
3. Logs show activity: `tail -50 logs/opalclaw.log`

If the group is not registered, it will be created automatically on the next OpalClaw restart.
...
```elisp
;; M-x describe-variable RET opalclaw-auth-token RET
```

Must exactly match `EMACS_AUTH_TOKEN` in `.env`.

### opalclaw.el not loading

Check the path is correct:

```bash
ls ~/src/opalclaw/emacs/opalclaw.el
```

If OpalClaw is cloned elsewhere, update the `load`/`load-file` path in your Emacs config.
...
```bash
# macOS:
launchctl unload ~/Library/LaunchAgents/com.opalclaw.plist
npm run dev
# When done testing:
launchctl load ~/Library/LaunchAgents/com.opalclaw.plist

# Linux:
# systemctl --user stop opalclaw
# npm run dev
# systemctl --user start opalclaw
```
...
1. Delete `src/channels/emacs.ts`, `src/channels/emacs.test.ts`, and `emacs/opalclaw.el`
2. Remove `import './emacs.js'` from `src/channels/index.ts`
3. Remove the OpalClaw block from your Emacs config file
...
6. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.opalclaw` (macOS) or `npm run build && systemctl --user restart opalclaw` (Linux)