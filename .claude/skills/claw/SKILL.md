---
name: claw
description: Install the claw CLI tool — run OpalClaw agent containers from the command line without opening a chat app.
---

# claw — OpalClaw CLI

`claw` is a Python CLI that sends prompts directly to a OpalClaw agent container from the terminal. It reads registered groups from the OpalClaw database, picks up secrets from `.env`, and pipes a JSON payload into a container run — no chat app required.
...
- Python 3.8 or later
- OpalClaw installed with a built and tagged container image (`opalclaw-agent:latest`)
- Either `container` (Apple Container, macOS 15+) or `docker` available in `PATH`

## Install

Run this skill from within the OpalClaw directory. The script auto-detects its location, so the symlink always points to the right place.
...
You should see registered groups. If OpalClaw isn't running or the database doesn't exist yet, the list will be empty — that's fine.
...
claw --image opalclaw-agent:dev "Hello"
...
The script auto-detects your OpalClaw directory and reads `.env` from it. Check that the file exists and contains at least one of: `CLAUDE_CODE_OAUTH_TOKEN`, `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`.

### Container times out

The default timeout is 300 seconds. For longer tasks, pass `--timeout 600` (or higher). If the container consistently hangs, check that your `opalclaw-agent:latest` image is up to date by running `./container/build.sh`.
...
### Override the OpalClaw directory

If `claw` can't find your database or `.env`, set the `OPALCLAW_DIR` environment variable:

```bash
export OPALCLAW_DIR=/path/to/your/opalclaw
```
