...
This skill replaces the OneCLI gateway with OpalClaw's built-in credential proxy. Containers get credentials injected via a local HTTP proxy that reads from `.env` — no external services needed.
...
If neither check matches, you may be on an older version. Run `/update-opalclaw` first, then retry.
...
- macOS: `launchctl kickstart -k gui/$(id -u)/com.opalclaw`
...
```bash
tail -20 logs/opalclaw.log | grep "Credential proxy"
```
