---
name: x-integration
description: X (Twitter) integration for OpalClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** OpalClaw v1.0.0. Directory structure may change in future versions.
...
1. **OpalClaw is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep opalclaw (macOS) or systemctl --user status opalclaw (Linux)
```
...
| `OPALCLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/opalclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into OpalClaw, make the following modifications:
...
All paths below are relative to project root (`OPALCLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep opalclaw  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status opalclaw
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/opalclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/opalclaw.log | tail -20
```
...
docker run opalclaw-agent ls -la /app/src/skills/
```
