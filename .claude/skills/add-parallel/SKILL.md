...
Adds Parallel AI MCP integration to OpalClaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses OpalClaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. OpalClaw already set up and running
...
Add Parallel AI MCP servers after the opalclaw server:
```typescript
const mcpServers: Record<string, any> = {
  opalclaw: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__opalclaw__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__opalclaw__send_message
   4. Use mcp__opalclaw__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo opalclaw-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep opalclaw  # macOS
...
- Check task runs: `tail -f logs/opalclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.opalclaw` (macOS) or `systemctl --user restart opalclaw` (Linux)
